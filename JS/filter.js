...
function applyFilters() {
  let items = bergenAirConditioners.slice();
...
function renderList(items) {
  ...
  items.forEach(item => {
    const cool  = item.generalCharacteristics['Холодовиробництво, кВт'];
    const heat  = item.generalCharacteristics['Тепловиробництво, кВт'];
    ...
    li.innerHTML = `
      ...
      <div class="item-info-wrapper">
        ...
        <p class="min-tec-spec">Холодовиробництво, кВт ${cool}</p>
        <p class="min-tec-spec">Потужність обогріву, кВт ${heat}</p>
...
function onAddToCartClick(event) {
  ...
  const item = bergenAirConditioners.find(i => i.id === id);
...
restoreFormControls();
renderList(bergenAirConditioners);
applyFilters();
